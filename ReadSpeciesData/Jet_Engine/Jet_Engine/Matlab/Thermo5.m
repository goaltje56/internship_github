...
h1 = YF.*HNasa(TA,Sp(55))+YO_before.*HNasa(TA,Sp(4))+YN2.*HNasa(TA,Sp(48)); %% line 58

%% h2 for every temperature and mixture
i=1;
for T=200:1:3000
    for j = 1:length(AF)
        h2(i,j) = YCO2(j)*HNasa(T,Sp(16))+YH2O(j)*HNasa(T,Sp(6))+YN2(j)*HNasa(T,Sp(48))+YO2after(j)*HNasa(T,Sp(4)); %% line 64
    end
    i=i+1;
end
...
for i=1:length(AF)                                                                % Compute properties for all species for TR temperature range
    T_ad(i) = interp1(h2(:,i),T_i,h1(i));           %%line 71
end
