...
iMr  = find(strcmpi('Mr',B.colheaders));
iMp  = find(strcmpi('Mp',B.colheaders));
iYp1 = find(strcmpi('Yr1',B.colheaders));
iYp2 = find(strcmpi('Yr2',B.colheaders));
iYp3 = find(strcmpi('Yr3',B.colheaders));
iYp4 = find(strcmpi('Yr4',B.colheaders));
icut  = find(strcmpi('stagecut',B.colheaders));
...
Mr_new  = reshape(Mr,[row, col]);
Mp_new  = reshape(Mp,[row, col]);
Yr1_new = reshape(Yr1,[row, col]);
Yr2_new = reshape(Yr2,[row, col]);
Yr3_new = reshape(Yr3,[row, col]);
Yr4_new = reshape(Yr4,[row, col]);
cut_new = reshape(cut,[row, col]);
% plot the results
% figure(1)
% hold on
...
%     fields(i);
%     path_Results = strcat('C:\Users\s137280\Documents\Master_tue\Internship\internship_github\Species_sink\results_10_11_2018\massflow',num2str(i),'.avi');
...
% for i = 1:numel(fields)
%     fields(i);
%     path_Results = strcat('C:\Users\s137280\Documents\Master_tue\Internship\internship_github\Species_sink\results_10_11_2018\Y',num2str(i),'.avi');
%     filename = VideoWriter(path_Results)
%     filename.FrameRate = 5;
%     
%     path_Results2 = strcat('C:\Users\s137280\Documents\Master_tue\Internship\internship_github\Species_sink\results_10_11_2018\X',num2str(i),'.avi');
