...
istagecut = find(strcmpi('stagecut',C.colheaders));
...
stagecut_new = reshape(stagecut,[row2, col]);
% plot the results
% figure(1)
% hold on
...
%     fields(i);
    path_Results = strcat('C:\Users\s137280\Documents\Master_tue\Internship\internship_github\Toos_Simplified_3membranes\results\NEWW',num2str(i),'.avi');
    ...
    subplot(2,2,1)
    hold on
    p1 = plot(time_new(1,1:n), f1_new(1,1:n),'-r','LineWidth',1);
    p2 = plot(time_new(1,1:n), f2_new(1,1:n),'-b','LineWidth',1);
...
%     xticklabels({'0','20','40','60','80','100','120','140','160','180','200'})
    ...
    subplot(2,2,4)
    hold on
    p1 = plot(stagecut_new(2:row-1,n),X1_new(2:row-1,n),'-r','LineWidth',1);
    p2 = plot(stagecut_new(2:row-1,n),X2_new(2:row-1,n),'-b','LineWidth',1);
    ...
    legend(ax1,[p1;p2;p3;p4],'O_2','CO_2','H_2O','AR','Location','best') 
    legend('boxoff')
    axis([0 1 0 1]);
    xticks([0 1])
    xticklabels({'', ''});
    ...
    subplot(2,2,2)
    hold on
    p1 = plot(stagecut_new(2:row-1,n),f1_new(2:row-1,n),'-r','LineWidth',1);
    p2 = plot(stagecut_new(2:row-1,n),f2_new(2:row-1,n),'-b','LineWidth',1);
