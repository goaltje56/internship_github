...
[u, p, pc, pc2, T, mu, Cp, Gamma, d_u, b, SP, Su, relax_u, relax_pc, relax_T, relax_rho, relax_f, Dt, u_old, T_old, pc_old]...
 = param_init(NPI, u_in);
...
for time = 0:Dt:Total_time
    ...
    [p, u2, pc] = velcorr(NPI, pc, p, u, relax_pc, d_u);
  
    b_p = pcorr(NPI, rho,A, (u2-u), aP_u, aW_u, aE_u, x, x_u);
    % 2nd pressure correction (modified form of continuity equation)
    [aE_pc2, aW_pc2, aP_pc2, b_pc2, Istart_pc, pc2] = pccoeff2(NPI-1, rho, A, x, x_u,u2, d_u, rho_old, Dx, Dt, b_p, pc2);
    pc2 = solve_eq(NPI-1, aE_pc, aW_pc, aP_pc, b_pc, pc2, 2);
    
    [p2, u3] = velcorr2(NPI, pc, pc2, p, u, b_p, d_u);

    % Temperature
    [aE_T, aW_T, aP_T, b_T, Istart_T, aPold_T] = Tcoeff(NPI, rho, A, x, x_u, u3, T, Gamma, relax_T, Dt, T_old, Dx, aPold_T);
%     [TR, r_T] = GS_solve(NPI+1,T, aW_T, aE_T, aP_T, b_T, 10^(-6));
 	T = solve_eq(NPI,aE_T, aW_T, aP_T, b_T, T, 2);
    
%    Species 
    for i = 1:n
        [aE_f(i,:), aW_f(i,:), aP_f(i,:), b_f(i,:), Istart_f] = Fcoeff(NPI, rho, A, x, x_u, u3, Y_k(i,:), D_k(i,:), relax_f, Dt, f_old(i,:), Dx, rho_old);
        Y_k(i,:) = solve_eq(NPI,aE_f(i,:), aW_f(i,:), aP_f(i,:), b_f(i,:), Y_k(i,:), 2);
    end
    Y_k = species_bound(NPI, n, Y_k);
    [u3, T, Y_k, m_in, m_out, p] = bound(NPI,rho,x,x_u,A,u3, u_in, T, Y_k, p2);  

    end
    
    % store results of this run as old results for next iteration
    [u_old, pc_old, T_old, rho_old, f_old, p] = storeresults(NPI, u3, pc, T, rho, Y_k, u_old, pc_old, T_old, rho_old, f_old, p2, n);
