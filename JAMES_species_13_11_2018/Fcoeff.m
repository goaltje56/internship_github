function [aE aW aP b Istart_F Y_sink] = Fcoeff(NPI, rho,rho_s, A, x, x_u, u, Y_k, Y2_k,T, rho_real, rho2_real, P_k, D, relax_f, Dt, f_old, Dx, rho_old, sink, n);
    Istart_F = 2;
...
%         
        ...
        if sink == 1
            SP(I) = 0;
            Su(I) = -P_k(I)*(rho_real(i)*Runi*T(I)*Y_k(I)-rho2_real(I)*Runi*T(I)*Y2_k(I));      % sink term depends on mass fraction
            Y_sink(I) = -P_k(I)*(rho_real(i)*Runi*T(I)*Y_k(I)-rho2_real(I)*Runi*T(I)*Y2_k(I));
        else 
        SP(I) = 0;
        Su(I) = 0;
        Y_sink(I) = 0;
